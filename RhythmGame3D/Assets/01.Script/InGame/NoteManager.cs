...
public class NoteManager : MonoBehaviour
{
    ...
    Queue<GameObject> ObjectPool = new Queue<GameObject>(), LongObjectPool = new Queue<GameObject>(); //오브젝트를 담을 큐
    ...
    private void Start()
    {
        if (null == instance)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                CreateLongObject(); //초기에 10개의 오브젝트를 생성함
            }
        }
        else
        {
    ...
    GameObject CreateLongObject() //초기 OR 오브젝트 풀에 남은 오브젝트가 부족할 때, 오브젝트를 생성하기위해 호출되는 함수
    {
    ...
    public GameObject LongGetObject() //오프젝트가 필요할 때 다른 스크립트에서 호출되는 함수
    {
        if (LongObjectPool.Count > 0) //현재 큐에 남아있는 오브젝트가 있다면,
        {
    ...
    public void LongReturnObjectToQueue(GameObject obj) //사용이 완료 된 오브젝트를 다시 큐에 넣을때 호출 파라미터->비활성화 할 오브젝트
    {
        obj.gameObject.SetActive(false);
        obj.transform.SetParent(instance.transform);
        instance.LongObjectPool.Enqueue(obj); //다시 큐에 넣음
    }
}
